...
const NAV_ITEMS: NavItem[] = [
  ...
  { label: 'Bank Transfers', href: '/admin/bank-transfers', icon: Landmark },
...
export default function AdminSidebar({
  ...
  const navWithBadges: NavItem[] = NAV_ITEMS.map((item) => {
    if (item.href === '/admin/orders')
      return { ...item, badge: pendingOrders || undefined };
    if (item.href === '/admin/bank-transfers')
      return { ...item, badge: pendingTransfers || undefined };
    return item;
  });
...
function LogoutButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-[0.60rem] tracking-[0.10em] font-medium">
        Log Out
      </span>
    </button>
  );
