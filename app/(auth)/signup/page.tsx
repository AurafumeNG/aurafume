...
function Field({
  id, label, type = 'text', value, onChange, onBlur, error, placeholder, autoComplete,
  required = true, right,
}: {
...
export default function SignupPage() {
  ...
  function validateField(field: string, val: string): string | undefined {
    switch (field) {
      case 'firstName': return !val.trim() ? 'First name is required' : undefined;
      case 'lastName':  return !val.trim() ? 'Last name is required' : undefined;
      case 'email':
  ...
  function validateAll() {
    ...
    (['firstName', 'lastName', 'email', 'password', 'confirm'] as const).forEach(f => {
    ...
    return (
      ...
      >
        ...
        <p className="text-[0.52rem] tracking-[0.1em] text-muted-foreground/50">
          Already verified?{' '}
          <Link href="/login" className="text-muted-foreground hover:text-foreground underline underline-offset-2 transition-colors">
            Sign in
          </Link>
        </p>
      </motion.div>
  ...
  return (
    <div className="space-y-7">
      <div>
        <h1 className="text-[0.62rem] tracking-[0.3em] uppercase text-foreground font-medium">Create Account</h1>
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field id={firstId} label="First Name" value={form.firstName} autoComplete="given-name"
            onChange={v => set('firstName', v)} onBlur={() => blur('firstName', form.firstName)}
            error={errors.firstName} placeholder="Ada"
          />
          <Field id={lastId} label="Last Name" value={form.lastName} autoComplete="family-name"
            onChange={v => set('lastName', v)} onBlur={() => blur('lastName', form.lastName)}
            error={errors.lastName} placeholder="Obi"
          />
        </div>

        <Field id={emailId} label="Email" type="email" value={form.email} autoComplete="email"
          onChange={v => set('email', v)} onBlur={() => blur('email', form.email)}
          error={errors.email} placeholder="you@example.com"
        />

        <Field id={phoneId} label="Phone" type="tel" value={form.phone} autoComplete="tel"
          onChange={v => set('phone', v)} onBlur={() => {}} required={false}
          placeholder="+234 800 000 0000"
        />

        {/* Password */}
        <div>
          <Field id={pwId} label="Password" type={showPw ? 'text' : 'password'} value={form.password}
            autoComplete="new-password"
            onChange={v => set('password', v)} onBlur={() => blur('password', form.password)}
        ...
        <Field id={confirmId} label="Confirm Password" type={showConfirm ? 'text' : 'password'}
          value={form.confirm} autoComplete="new-password"
          onChange={v => set('confirm', v)} onBlur={() => blur('confirm', form.confirm)}
        ...
        <div className="space-y-1">
          <label htmlFor={termsId} className="flex items-start gap-3 cursor-pointer group">
            <div className="mt-0.5 relative">
              <input
                id={termsId} type="checkbox" checked={agreed}
              ...
              <div className={`w-4 h-4 border flex items-center justify-center transition-colors duration-150 ${
                agreed ? 'border-accent bg-accent/20' : 'border-border group-hover:border-accent/50'
              }`}>
                {agreed && (
                  <motion.svg initial={{ scale: 0 }} animate={{ scale: 1 }} width="9" height="9" viewBox="0 0 9 9">
                    <polyline points="1,4.5 3.5,7 8,1.5" fill="none" stroke="oklch(0.72 0.10 74)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  </motion.svg>
                )}
              </div>
            </div>
            <span className="text-[0.56rem] tracking-[0.06em] leading-relaxed text-muted-foreground">
              I agree to the{' '}
              <Link href="/terms" className="text-foreground underline underline-offset-2 hover:text-accent transition-colors">
                Terms of Service
              </Link>{' '}and{' '}
              <Link href="/privacy" className="text-foreground underline underline-offset-2 hover:text-accent transition-colors">
                Privacy Policy
              </Link>
            </span>
          </label>
        ...
        >
          {isLoading
            ? <><Loader2 size={13} strokeWidth={2.2} className="animate-spin" /><span>Creating account…</span></>
            : 'Create Account'
          }
        </button>
      </form>
