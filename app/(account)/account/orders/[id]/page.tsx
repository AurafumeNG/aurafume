...
import {
  ChevronLeft, Check, Clock, Package, Truck, MapPin,
  CreditCard, Building2, Download, ShoppingBag,
...
const STATUS_META: Record<OrderStatus, { label: string; className: string }> = {
  pending:    { label: 'Pending Payment', className: 'bg-amber-500/10  text-amber-600  border-amber-500/20'  },
  processing: { label: 'Processing',      className: 'bg-blue-500/10   text-blue-600   border-blue-500/20'   },
  shipped:    { label: 'Shipped',          className: 'bg-violet-500/10 text-violet-600 border-violet-500/20' },
...
function StatusBadge({ status }: { status: OrderStatus }) {
  ...
  return (
    <span className={`inline-flex text-[0.48rem] tracking-[0.14em] uppercase font-semibold px-2.5 py-1 border ${className}`}>
      {label}
    </span>
  );
}
            ...
            <div key={key} className="flex items-start gap-4 py-3 relative">
              ...
              <motion.div
                ...
                style={{
                  ...
                  borderColor:  completed ? 'transparent' : isCancelled ? 'oklch(0.55 0.18 25 / 0.3)' : 'oklch(0.35 0 0)',
                  boxShadow:    current ? `0 0 0 3px oklch(0.72 0.10 74 / 0.25)` : 'none',
                }}
              >
                <Icon
                  size={14}
                  strokeWidth={completed ? 2.2 : 1.6}
                  style={{ color: completed ? 'oklch(0.12 0 0)' : isCancelled ? 'oklch(0.55 0.18 25 / 0.5)' : 'oklch(0.45 0 0)' }}
                />
              </motion.div>

              {/* Label */}
              <div className="pt-1.5">
                <p className={`text-[0.62rem] tracking-[0.1em] uppercase font-medium transition-colors ${
                  completed ? 'text-foreground' : 'text-muted-foreground/40'
                }`}>
                  {label}
                </p>
                {current && !isCancelled && (
                  <p className="text-[0.52rem] tracking-[0.06em] mt-0.5" style={{ color: GOLD }}>
                    Current status
                  </p>
                )}
              </div>
...
function generateInvoiceHTML(order: Order): string {
  const date  = order.date.toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
    ...
    >
      ...
      >
        <div className="space-y-1.5">
          <h3 className="text-[0.64rem] tracking-[0.2em] uppercase font-medium text-foreground">{title}</h3>
          <p className="text-[0.6rem] tracking-[0.04em] leading-relaxed text-muted-foreground">{body}</p>
        </div>
  ...
  return (
    ...
    >
      <div className="px-5 py-3 border border-border bg-background/95 backdrop-blur-sm shadow-lg max-w-xs">
        <p className="text-[0.6rem] tracking-[0.08em] text-foreground text-center">{message}</p>
      </div>
    </motion.div>
  );
}
...
export default function OrderDetailPage() {
  ...
  function handleReorder() {
    ...
    showToast(`${order.items.length} item${order.items.length > 1 ? 's' : ''} added to your cart`);
  }
  ...
  async function handleReturnConfirm() {
    ...
    setDialogLoading(false);
    setDialog(null);
    showToast("Return & refund request submitted. We'll be in touch within 24 hours.");
  }

  if (!order) {
    return (
      <div className="flex flex-col items-center justify-center gap-5 py-32 text-center px-8">
        <Package size={32} strokeWidth={1.3} className="text-muted-foreground/30" />
        <div className="space-y-1.5">
          <p className="text-[0.62rem] tracking-[0.22em] uppercase text-foreground">Order not found</p>
      ...
      <div className="pt-14 max-w-xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <Section title="Items Ordered">
          <div className="divide-y divide-border/40">
            {order.items.map((item, i) => (
              <div key={item.productId + i} className="flex items-center gap-4 px-4 py-4">
        ...
        {order.deliveryAddress && (
          <Section title="Delivery Address">
            <div className="px-4 py-4 flex items-start gap-3">
              <MapPin size={15} strokeWidth={1.7} className="mt-0.5 shrink-0" style={{ color: GOLD }} />
          ...
          <Section title="Payment Method">
            <div className="px-4 py-4 flex items-center gap-3">
              {order.paymentMethod === 'paystack'
                ? <CreditCard size={15} strokeWidth={1.7} style={{ color: GOLD }} />
                : <Building2  size={15} strokeWidth={1.7} style={{ color: GOLD }} />
              }
              <p className="text-[0.65rem] tracking-[0.06em] text-foreground">
                {order.paymentMethod === 'paystack' ? 'Paystack (Card / Bank / USSD)' : 'Bank Transfer'}
              </p>
            </div>
          </Section>
        )}
        ...
        <Section title="Price Breakdown">
          <div className="px-4 py-4 space-y-3">
            {[
              { label: 'Subtotal',                                value: `₦${order.subtotal.toLocaleString()}`,      muted: false },
              { label: 'Delivery',                               value: `₦${order.deliveryFee.toLocaleString()}`,   muted: false },
              ...(order.discount > 0
                ? [{ label: `Discount${order.couponCode ? ` (${order.couponCode})` : ''}`,
                     value: `−₦${order.discount.toLocaleString()}`, muted: false, accent: true }]
                : []),
            ].map(({ label, value, accent }) => (
              <div key={label} className="flex items-center justify-between">
                <span className="text-[0.58rem] tracking-[0.1em] text-muted-foreground/70">{label}</span>
                <span className={`text-[0.68rem] tabular-nums ${accent ? 'text-green-600 font-medium' : 'text-foreground'}`}>
                  {value}
                </span>
              </div>
            ))}
