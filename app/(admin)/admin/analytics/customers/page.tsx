...
import CustomerPageHeader from '@/components/admin/analytics/customers/CustomerPageHeader';
import CustomerKPIs from '@/components/admin/analytics/customers/CustomerKPIs';
import CustomerGrowthChart from '@/components/admin/analytics/customers/CustomerGrowthChart';
import NewVsReturning from '@/components/admin/analytics/customers/NewVsReturning';
import RetentionCohort from '@/components/admin/analytics/customers/RetentionCohort';
import LTVDistribution from '@/components/admin/analytics/customers/LTVDistribution';
import CustomerSegmentation from '@/components/admin/analytics/customers/CustomerSegmentation';
import PurchaseFrequency from '@/components/admin/analytics/customers/PurchaseFrequency';
import CustomerGeographic from '@/components/admin/analytics/customers/CustomerGeographic';
import DeviceRegistration from '@/components/admin/analytics/customers/DeviceRegistration';
import TopCustomersTable from '@/components/admin/analytics/customers/TopCustomersTable';
...
export default function CustomerAnalyticsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/analytics/customers?from=${from}&to=${to}`, { signal: ctrl.signal })
      .then((r) => r.json())
      .then((json: { data?: CustomersData }) => {
        if (!ctrl.signal.aborted) setCustomersData(json.data ?? null);
      })
  ...
  return (
    <div className="min-h-screen" style={{ background: '#0F0F0F' }}>
      ...
      <div className="lg:pl-55 flex flex-col min-h-screen">
        <AdminTopNav
          pageTitle="Customer Analytics" adminName={adminName} avatarUrl={adminUser?.avatar}
        ...
        <main className="flex-1 pt-14">
          <div className="p-5 md:p-7 space-y-6">
            ...
            {/* ── Sub-navigation tabs ──────────────────────────────── */}
            <AnalyticsTabs active="Customers" />
            ...
            {/* ── KPI Cards ────────────────────────────────────────── */}
            <section>
              <CustomerKPIs compare={compare} data={customersData} />
            </section>

            {/* ── Customer Growth ───────────────────────────────────── */}
            <section>
              <CustomerGrowthChart data={customersData} />
            </section>

            {/* ── New vs Returning ──────────────────────────────────── */}
            <section>
              <NewVsReturning data={customersData} />
            </section>

            {/* ── Retention Cohort ──────────────────────────────────── */}
            <section>
              <RetentionCohort data={customersData} />
            </section>

            {/* ── LTV Distribution ──────────────────────────────────── */}
            <section>
              <LTVDistribution data={customersData} />
            </section>

            {/* ── Customer Segmentation (RFM) ───────────────────────── */}
            <section>
              <CustomerSegmentation data={customersData} />
            </section>

            {/* ── Purchase Frequency + Geographic (side by side xl) ─── */}
            <section className="grid grid-cols-1 xl:grid-cols-2 gap-4">
              <PurchaseFrequency data={customersData} />
              <CustomerGeographic data={customersData} />
            </section>

            {/* ── Device & Registration ─────────────────────────────── */}
            <section>
              <DeviceRegistration data={customersData} />
            </section>
